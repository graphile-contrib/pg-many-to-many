...
Writing a custom inflector plugin gives you full control over the GraphQL field
names. A common customization is shortening the field names to just the table
name (producing e.g. `teams`):
...
This plugin does not currently include a V5 inflector example; for now, refer
to the [inflection documentation](https://postgraphile.org/postgraphile/next/inflection)
when writing a custom inflection plugin.
...
The `@manyToManyFieldName` smart comment allows you to override the field names
